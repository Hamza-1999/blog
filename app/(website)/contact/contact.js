...
export default function Contact({ settings }) {
  ...
  const handleFormSubmit = async (data) => {
    try {
      // setLoading(true);
      await emailjs.send("service_0kkx6wj", "template_appdw0b", {
       name: data.name,
       email : data.email,
  ...
  useEffect(() => emailjs.init("G63cp5tcsEyW9EjeU"), []);

  return (
    <Container>
      <h1 className="text-brand-primary mb-3 mt-2 text-center text-2xl font-semibold tracking-tight dark:text-white lg:text-4xl lg:leading-snug">
        Contact
      </h1>
      ...
      <div className="my-10 grid md:grid-cols-2">
        <div className="my-10">
          ...
          <p className="mt-5 max-w-sm">
            We value your feedback and strive to respond to all
            inquiries in a timely manner. We look forward to hearing
          ...
          <div className="mt-5">
            <div className="text-dark-600 mt-2 flex items-center space-x-2 dark:text-gray-400">
              ...
              <span>89747 Norbert Ville</span>
            </div>
            {/* {settings?.email && ( */}
            <div className="text-dark-600 mt-2 flex items-center space-x-2 dark:text-gray-400">
              <EnvelopeIcon className="h-4 w-4" />
              <a href={`mailto:quillcrafts@gmail.com`}>
                {/* {settings.email} */}
                quillcrafts@gmail.com
              </a>
            </div>
            {/* )} */}
            {settings?.phone && (
